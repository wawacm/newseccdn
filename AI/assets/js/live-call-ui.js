...
async switchCamera(){const btn=document.getElementById('cameraSwitchBtn');if(btn)btn.disabled=true;try{const success=await this.liveCall.switchCamera();if(success){this.showToast('摄像头已切换','success');}else{this.showToast('摄像头切换失败','error');}}catch(error){this.showToast('摄像头切换失败','error');}
