...
initCustomDropdown(){const dropdownSelected=document.getElementById('dropdownSelected');const dropdownOptions=document.getElementById('dropdownOptions');const modelSelect=document.getElementById('modelSelect');const currentModelDisplay=document.getElementById('currentModelDisplay');if(!dropdownSelected||!dropdownOptions)return;dropdownSelected.addEventListener('click',(e)=>{e.stopPropagation();dropdownSelected.classList.toggle('open');dropdownOptions.classList.toggle('open');});dropdownOptions.addEventListener('click',(e)=>{const option=e.target.closest('.dropdown-option');if(!option||!dropdownOptions.contains(option)){return;}
e.stopPropagation();const value=option.dataset.value;const text=option.textContent.split(' (')[0];dropdownSelected.querySelector('span').textContent=text;if(currentModelDisplay){currentModelDisplay.textContent=text;}
...
async checkAuth(){if(this.accessControl&&!this.accessControl.isLoggedIn&&this.accessControl.isPublicDomain&&!this.accessControl.isApiDomain){this.currentUser=null;this.modelPolicy=this.accessControl.modelPolicy||null;if(!this.modelPolicy){this.modelPolicy=await this.fetchModelPolicyFallback();}
this.showChatPage();return;}
try{const response=await this.apiCall('/auth/user');if(response.success){this.currentUser=response.user;this.modelPolicy=response.model_policy||null;if(!this.modelPolicy&&this.accessControl){this.modelPolicy=this.accessControl.modelPolicy||null;}
await this.handlePendingSocialBindCallback();this.showChatPage();this.loadSessions();}else{if(this.accessControl&&this.accessControl.isPublicDomain&&!this.accessControl.isApiDomain){this.currentUser=null;this.modelPolicy=this.accessControl.modelPolicy||null;if(!this.modelPolicy){this.modelPolicy=await this.fetchModelPolicyFallback();}
this.showChatPage();}else{this.showLoginPage();}}}catch(error){if(this.accessControl&&this.accessControl.isPublicDomain&&!this.accessControl.isApiDomain){this.currentUser=null;this.modelPolicy=this.accessControl.modelPolicy||null;if(!this.modelPolicy){this.modelPolicy=await this.fetchModelPolicyFallback();}
this.showChatPage();}else{this.showLoginPage();}}}
async fetchModelPolicyFallback(){const endpoints=['api/auth/check-access','api/models'];for(const endpoint of endpoints){try{const response=await fetch(endpoint,{method:'GET',headers:{'Cache-Control':'no-cache, no-store, must-revalidate','Pragma':'no-cache','Expires':'0'},credentials:'same-origin'});if(!response.ok){continue;}
const contentType=response.headers.get('content-type')||'';if(!contentType.includes('application/json')){continue;}
const data=await response.json();if(data&&data.model_policy&&Array.isArray(data.model_policy.allowed_model_ids)){return data.model_policy;}}catch(error){}}
return null;}
...
this.currentUser=null;this.modelPolicy=null;this.currentSession=null;this.attachments=[];this.isLoading=false;this.clearUserData();this.resetAllUIElements();this.showNotification('登录已过期，请重新登录','warning');setTimeout(()=>{this.showLoginPage();},1500);}
...
updateModelSelector(){const modelSelect=document.getElementById('modelSelect');const dropdownOptions=document.getElementById('dropdownOptions');const searchToggle=document.getElementById('searchToggle');if(!modelSelect){return;}
const policy=this.getResolvedModelPolicy();const allowedModelSet=policy&&Array.isArray(policy.allowed_model_ids)?new Set(policy.allowed_model_ids.map((id)=>String(id))):null;const policyDefaultModel=(policy&&typeof policy.default_model_id==='string'&&policy.default_model_id.trim()!=='')?policy.default_model_id.trim():'wawa-ai-auto';const canUseProFeatures=this.canUseAdvancedModels();const policyModelMap=new Map();if(policy&&Array.isArray(policy.available_models)){policy.available_models.forEach((item)=>{if(!item||typeof item!=='object'){return;}
const modelId=String(item.id||'').trim();if(!modelId){return;}
const modelLabel=String(item.label||modelId).trim()||modelId;policyModelMap.set(modelId,modelLabel);});}
if(policyModelMap.size>0){policyModelMap.forEach((modelLabel,modelId)=>{const selectOption=Array.from(modelSelect.options).find((opt)=>opt.value===modelId);if(selectOption){selectOption.textContent=modelLabel;}else{const optionEl=document.createElement('option');optionEl.value=modelId;optionEl.textContent=modelLabel;modelSelect.appendChild(optionEl);}
if(dropdownOptions){const dropdownOption=Array.from(dropdownOptions.querySelectorAll('.dropdown-option')).find((opt)=>(opt.dataset.value||'')===modelId);if(dropdownOption){dropdownOption.textContent=modelLabel;}else{const optionDiv=document.createElement('div');optionDiv.className='dropdown-option';optionDiv.dataset.value=modelId;optionDiv.textContent=modelLabel;dropdownOptions.appendChild(optionDiv);}}});}
Array.from(modelSelect.options).forEach((option)=>{if(policyModelMap.has(option.value)){option.textContent=policyModelMap.get(option.value);}
const isAutoModel=option.value==='wawa-ai-auto';const isAllowed=allowedModelSet?allowedModelSet.has(option.value):(canUseProFeatures||isAutoModel);option.disabled=!isAllowed;option.hidden=!isAllowed;});if(dropdownOptions){dropdownOptions.querySelectorAll('.dropdown-option').forEach((option)=>{const modelValue=option.dataset.value||'';if(policyModelMap.has(modelValue)){option.textContent=policyModelMap.get(modelValue);}
const isAutoModel=modelValue==='wawa-ai-auto';const isAllowed=allowedModelSet?allowedModelSet.has(modelValue):(canUseProFeatures||isAutoModel);option.style.display=isAllowed?'':'none';});}
if(searchToggle){const searchToggleLabel=searchToggle.closest('.search-toggle');if(!canUseProFeatures&&searchToggle.checked){searchToggle.checked=false;this.searchAutoEnabled=false;}
searchToggle.disabled=!canUseProFeatures;searchToggle.title=canUseProFeatures?'启用联网搜索':'仅 Pro 用户可使用联网搜索';if(searchToggleLabel){searchToggleLabel.title=canUseProFeatures?'启用联网搜索':'仅 Pro 用户可使用联网搜索';searchToggleLabel.style.opacity=canUseProFeatures?'':'0.55';searchToggleLabel.style.cursor=canUseProFeatures?'':'not-allowed';}}
const currentModel=modelSelect.value||policyDefaultModel;let fallbackModel=this.isModelSelectableByCurrentUser(currentModel)?currentModel:policyDefaultModel;if(!this.isModelSelectableByCurrentUser(fallbackModel)){const firstAllowed=Array.from(modelSelect.options).find((option)=>!option.disabled&&!option.hidden);fallbackModel=firstAllowed?firstAllowed.value:'wawa-ai-auto';}
if(modelSelect.value!==fallbackModel){modelSelect.value=fallbackModel;}
this.syncModelDropdownSelection(fallbackModel);this.updateModelDisplay({target:{value:fallbackModel}});}
canUseAdvancedModels(){const policy=this.getResolvedModelPolicy();if(policy&&typeof policy.is_pro!=='undefined'){return!!policy.is_pro;}
return!!(this.currentUser&&Number(this.currentUser.can_use_pro)>0);}
isModelSelectableByCurrentUser(modelValue){const value=typeof modelValue==='string'?modelValue.trim():'';if(value===''){return false;}
const policy=this.getResolvedModelPolicy();if(policy&&Array.isArray(policy.allowed_model_ids)){return policy.allowed_model_ids.includes(value);}
if(value==='wawa-ai-auto'){return true;}
return this.canUseAdvancedModels();}
getResolvedModelPolicy(){if(this.modelPolicy&&Array.isArray(this.modelPolicy.allowed_model_ids)){return this.modelPolicy;}
if(this.accessControl&&this.accessControl.modelPolicy&&Array.isArray(this.accessControl.modelPolicy.allowed_model_ids)){return this.accessControl.modelPolicy;}
return null;}
getDefaultSelectableModel(){const policy=this.getResolvedModelPolicy();const policyDefault=(policy&&typeof policy.default_model_id==='string'&&policy.default_model_id.trim()!=='')?policy.default_model_id.trim():'wawa-ai-auto';if(this.isModelSelectableByCurrentUser(policyDefault)){return policyDefault;}
if(policy&&Array.isArray(policy.allowed_model_ids)&&policy.allowed_model_ids.length>0){return String(policy.allowed_model_ids[0]).trim();}
return 'wawa-ai-auto';}
syncModelDropdownSelection(modelValue){const dropdownSelected=document.getElementById('dropdownSelected');const dropdownOptions=document.getElementById('dropdownOptions');const currentModelDisplay=document.getElementById('currentModelDisplay');if(!dropdownOptions){return;}
let matchedOption=null;dropdownOptions.querySelectorAll('.dropdown-option').forEach((option)=>{const isMatched=option.dataset.value===modelValue;option.classList.toggle('selected',isMatched);if(isMatched){matchedOption=option;}});if(!matchedOption){matchedOption=dropdownOptions.querySelector('.dropdown-option[data-value="wawa-ai-auto"]');}
const modelText=matchedOption?matchedOption.textContent.split(' (')[0].trim():'Wawa AI Auto';if(dropdownSelected){const selectedTextEl=dropdownSelected.querySelector('span');if(selectedTextEl){selectedTextEl.textContent=modelText;}}
if(currentModelDisplay){currentModelDisplay.textContent=modelText;}}
...
async logout(){try{await this.apiCall('/auth/logout');this.clearUserData();this.currentUser=null;this.modelPolicy=null;this.resetAllUIElements();this.showNotification('已退出登录','success');this.showLoginPage();}catch(error){this.showNotification(error.message,'error');}}
resetAllUIElements(){const modelSelect=document.getElementById('modelSelect');const defaultModel=this.getDefaultSelectableModel();if(modelSelect){modelSelect.value=defaultModel;}
this.syncModelDropdownSelection(defaultModel);this.updateModelDisplay({target:{value:defaultModel}});this.updateAttachmentPreview();this.closeMobileSidebar();}
...
updateModelDisplay(e){const modelValue=e.target.value;const modelDisplayElement=document.getElementById('currentModelDisplay');const modelSelect=document.getElementById('modelSelect');if(modelDisplayElement){const modelNames={'wawa-ai-auto':'Wawa AI Auto'};let fallbackName='Wawa AI Auto';if(modelSelect){const selectedOption=modelSelect.options[modelSelect.selectedIndex];if(selectedOption&&selectedOption.textContent){fallbackName=selectedOption.textContent.split(' (')[0].trim();}}
modelDisplayElement.textContent=modelNames[modelValue]||fallbackName;}}
...
let thinkingMessageId=null;let messageSendSuccess=false;try{const model=document.getElementById('modelSelect').value;if(!this.isModelSelectableByCurrentUser(model)){const modelSelect=document.getElementById('modelSelect');const defaultModel=this.getDefaultSelectableModel();if(modelSelect){modelSelect.value=defaultModel;}
this.syncModelDropdownSelection(defaultModel);this.updateModelDisplay({target:{value:defaultModel}});this.showNotification('当前账号无权使用该模型，已切换到默认模型','warning');this.isLoading=false;this.updateSendButton();this.disableUserInput(false);return;}
if(model==='wawa-image-1'||model==='wawaimage'||model==='wawa-image-2'||model==='wawa-image-3'){await this.generateImage(requestMessage||displayMessage);this.isLoading=false;this.updateSendButton();this.disableUserInput(false);return;}
...
handleModelChange(e){const modelSelect=document.getElementById('modelSelect');if(!modelSelect){return;}
const selectedModel=(e&&e.target&&e.target.value)?e.target.value:modelSelect.value;if(!this.isModelSelectableByCurrentUser(selectedModel)){const defaultModel=this.getDefaultSelectableModel();modelSelect.value=defaultModel;this.syncModelDropdownSelection(defaultModel);this.showNotification('当前账号无权选择该模型，已切换到默认模型','warning');return;}
this.syncModelDropdownSelection(selectedModel);}
